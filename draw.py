...
def draw(front: str = "Blue", back: str = "Archive", dx: int = 76, dy: int = -7) -> Image:
    # consts and calc
    fnt_size = 84
    ...
    front_pic = front_pic.transform(
        (pic_width, pic_height), Image.AFFINE, data, Image.BICUBIC
    )
    ...
    pic.paste(back_pic, (front_pic.size[0], fnt_size-2), back_pic)# magic number dkw just -6
    pic.paste(prepaint_pic, (front_pic.size[0]-dx, dy), prepaint_pic)
    pic = pic.crop(pic.getbbox())
    pic.convert("RGB").show()
    return pic
